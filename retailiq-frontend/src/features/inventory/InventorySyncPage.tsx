...
import type { ChainTransfer } from '@/api/chain';
...
export default function InventorySyncPage() {
  ...
  const snapshot = snapshotQuery.data && 'snapshot' in snapshotQuery.data ? snapshotQuery.data.snapshot : null;
  ...
  const transferColumns = useMemo<Column<ChainTransfer>[]>(() => [
    ...
    { key: 'qty', header: 'Qty', render: (row) => Number(row.qty ?? 0).toLocaleString() },
        ...
        <CardContent className="space-y-4">
          {chainId ? (
            chainDashboardQuery.isLoading || transfersQuery.isLoading ? (
              ...
              <div className="grid gap-3 md:grid-cols-3">
                  <Info label="Chain stores" value={String(chainDashboardQuery.data?.per_store_today?.length ?? 0)} />
                  <Info label="Transfers" value={String(transfersQuery.data?.length ?? 0)} />
                  <Info label="Revenue today" value={`${Number(chainDashboardQuery.data?.total_revenue_today ?? 0).toLocaleString()}`} />
                </div>
                <DataTable
